<div class="animate-fade-in">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="font-display text-2xl font-semibold text-foreground">{{ __('admin.shape_flavors.title') }}</h2>
            <p class="mt-1 text-sm text-foreground-muted">{{ __('admin.shape_flavors.subtitle') }}</p>
        </div>
        @can('update shapes')
            <button wire:click="openCreate" class="btn-base bg-primary text-primary-foreground hover:bg-primary-hover">
                ...
                {{ __('admin.shape_flavors.add_combo') }}
            </button>
        @endcan
    </div>
    ...
    <div class="mb-4 flex flex-col gap-4 sm:flex-row">
        <input wire:model.live.debounce.300ms="search" type="text"
            placeholder="{{ __('admin.shape_flavors.search_placeholder') }}" class="input-base w-full sm:max-w-sm" />
        <select wire:model.live="filter_shape_id" class="select-base w-full sm:max-w-xs">
            <option value="">{{ __('admin.shape_flavors.all_shapes') }}</option>
            @foreach ($shapes as $shape)
                <option value="{{ $shape->id }}">{{ $shape->name }}</option>
            @endforeach
        </select>
        <select wire:model.live="filter_flavor_id" class="select-base w-full sm:max-w-xs">
            <option value="">{{ __('admin.shape_flavors.all_flavors') }}</option>
            @foreach ($flavors as $flavor)
                <option value="{{ $flavor->id }}">{{ $flavor->name }}</option>
            @endforeach
        </select>
        @if ($search || $filter_shape_id || $filter_flavor_id)
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-border bg-surface-alt">
                        <th class="table-header px-6 py-3.5">{{ __('admin.shape_flavors.table_shape') }}</th>
                        <th class="table-header px-6 py-3.5">{{ __('admin.shape_flavors.table_flavor') }}</th>
                        <th class="table-header px-6 py-3.5">{{ __('admin.shape_flavors.table_price') }}</th>
                        <th class="table-header px-6 py-3.5">{{ __('admin.shape_flavors.table_images') }}</th>
                        <th class="table-header px-6 py-3.5 text-end">{{ __('admin.common.actions') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-border">
                    @forelse ($combos as $combo)
                        <tr wire:key="sf-{{ $combo->id }}" class="hover:bg-surface-alt/50 transition-colors duration-150">
                            <td class="px-6 py-4 text-sm font-medium text-foreground">{{ $combo->shape->name ?? '—' }}
                            </td>
                            <td class="px-6 py-4 text-sm text-foreground">{{ $combo->flavor->name ?? '—' }}</td>
                            <td class="px-6 py-4 text-sm text-foreground-muted">${{ number_format($combo->extra_price, 2) }}
                            </td>
                            <td class="px-6 py-4">
                                <div class="flex gap-2">
                                    @foreach (['full_image', 'cut_image'] as $col)
                                        <div class="relative group">
                                            @if ($url = $combo->getFirstMediaUrl($col))
                                                <img src="{{ $url }}" alt="{{ $col }}"
                                                    class="h-10 w-10 rounded-xl object-cover border border-border" />
                                            @else
                                                <div
                                            ...
                                            @endif
                                            <div
                                                class="absolute bottom-full start-1/2 mb-2 hidden -translate-x-1/2 whitespace-nowrap rounded bg-espresso px-2 py-1 text-xs text-white group-hover:block z-10">
                                                {{ __('admin.shape_flavors.' . $col . '_label') ?? Str::title(str_replace('_', ' ', $col)) }}
                                            </div>
                                        </div>
                                    @endforeach
                                </div>
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-sm text-foreground-muted">
                                {{ __('admin.shape_flavors.no_combos') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div
                ...
                <h3 class="font-display text-lg font-semibold text-foreground mb-5">
                    {{ $editingId ? __('admin.shape_flavors.edit_title') : __('admin.shape_flavors.create_title') }}
                </h3>
                <form wire:submit="save" class="space-y-4">
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <label
                                class="mb-1.5 block text-sm font-medium text-foreground">{{ __('admin.shape_flavors.shape_label') }}</label>
                            <select wire:model="cake_shape_id" class="select-base">
                                <option value="">{{ __('admin.shape_flavors.shape_placeholder') }}</option>
                                @foreach ($shapes as $shape)
                                    <option value="{{ $shape->id }}">{{ $shape->name }}</option>
                                @endforeach
                            </select>
                        ...
                        <div>
                            <label
                                class="mb-1.5 block text-sm font-medium text-foreground">{{ __('admin.shape_flavors.flavor_label') }}</label>
                            <select wire:model="cake_flavor_id" class="select-base">
                                <option value="">{{ __('admin.shape_flavors.flavor_placeholder') }}</option>
                                @foreach ($flavors as $flavor)
                                    <option value="{{ $flavor->id }}">{{ $flavor->name }}</option>
                                @endforeach
                            </select>
                            @error('cake_flavor_id') <p class="mt-1 text-xs text-danger">{{ $message }}</p> @enderror
                        </div>
                    </div>
                    <div>
                        <label
                            class="mb-1.5 block text-sm font-medium text-foreground">{{ __('admin.shape_flavors.price_label', ['currency' => settings(\App\Settings\CurrencySettings::class)->currency_symbol]) }}</label>
                    ...
                    <div class="grid grid-cols-2 gap-4">
                        <div>
                            <x-media-upload model="full_image" label="{{ __('admin.shape_flavors.full_image_label') }}"
                                hint="{{ __('admin.shape_flavors.full_image_hint') }}"
                                :preview="$editingModel?->getFirstMediaUrl('full_image')"
                                remove-signal="delete_full_image" />
                        </div>
                        <div>
                            <x-media-upload model="cut_image" label="{{ __('admin.shape_flavors.cut_image_label') }}"
                                hint="{{ __('admin.shape_flavors.cut_image_hint') }}"
                                :preview="$editingModel?->getFirstMediaUrl('cut_image')" remove-signal="delete_cut_image" />
                        </div>
                    </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-sm rounded-2xl bg-surface p-6 shadow-modal animate-scale-in">
                ...
                <h3 class="text-center font-display text-lg font-semibold text-foreground">
                    {{ __('admin.shape_flavors.delete_title') }}
                </h3>
                <p class="mt-2 text-center text-sm text-foreground-muted">{{ __('admin.shape_flavors.delete_message') }}</p>
