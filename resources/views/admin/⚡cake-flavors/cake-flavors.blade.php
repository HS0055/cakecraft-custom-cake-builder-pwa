<div class="animate-fade-in">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="font-display text-2xl font-semibold text-foreground">{{ __('admin.cake_flavors.title') }}</h2>
            <p class="mt-1 text-sm text-foreground-muted">{{ __('admin.cake_flavors.subtitle') }}</p>
        </div>
        @can('create flavors')
            <button wire:click="openCreate" class="btn-base bg-primary text-primary-foreground hover:bg-primary-hover">
                ...
                {{ __('admin.cake_flavors.add_flavor') }}
            </button>
        @endcan
    </div>
    ...
    <div class="mb-4">
        <input wire:model.live.debounce.300ms="search" type="text"
            placeholder="{{ __('admin.cake_flavors.search_placeholder') }}" class="input-base w-full sm:max-w-sm" />
    </div>
    ...
    <div class="card-base overflow-hidden">
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-border bg-surface-alt">
                        <th class="table-header px-6 py-3.5">{{ __('admin.cake_flavors.table_thumbnail') }}</th>
                        <th class="table-header px-6 py-3.5">{{ __('admin.cake_flavors.table_name') }}</th>
                        <th class="table-header px-6 py-3.5">{{ __('admin.cake_flavors.table_created') }}</th>
                        <th class="table-header px-6 py-3.5 text-end">{{ __('admin.common.actions') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-border">
                    @forelse ($flavors as $flavor)
                        <tr wire:key="flavor-{{ $flavor->id }}"
                            class="hover:bg-surface-alt/50 transition-colors duration-150">
                            <td class="px-6 py-4">
                                @if ($flavor->getFirstMediaUrl('thumbnail'))
                                    <img src="{{ $flavor->getFirstMediaUrl('thumbnail') }}" alt="{{ $flavor->name }}"
                            ...
                            <td class="px-6 py-4 text-sm font-medium text-foreground">{{ $flavor->name }}</td>
                            <td class="px-6 py-4 text-sm text-foreground-muted">{{ $flavor->created_at->format('M d, Y') }}
                            </td>
                            <td class="px-6 py-4 text-end">
                                <div class="flex items-center justify-end gap-1">
                                    @can('update flavors')
                                        <button wire:click="openEdit({{ $flavor->id }})"
                                    ...
                                    @endcan
                                    @can('delete flavors')
                                        <button wire:click="confirmDelete({{ $flavor->id }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center text-sm text-foreground-muted">
                                {{ __('admin.cake_flavors.no_flavors') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if ($flavors->hasPages())
            <div class="border-t border-border px-6 py-4">{{ $flavors->links() }}</div>
        @endif
    </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-md rounded-2xl bg-surface p-6 shadow-modal animate-scale-in">
                <h3 class="font-display text-lg font-semibold text-foreground mb-5">
                    {{ $editingId ? __('admin.cake_flavors.edit_title') : __('admin.cake_flavors.create_title') }}
                </h3>
                <form wire:submit="save" class="space-y-4">
                    @php
                        $editingModel = $editingId ? $flavors->firstWhere('id', $editingId) : null;
                    @endphp
                    <div>
                        <label
                            class="mb-1.5 block text-sm font-medium text-foreground">{{ __('admin.cake_flavors.name_label') }}</label>
                        <input wire:model="name" type="text" class="input-base"
                            placeholder="{{ __('admin.cake_flavors.name_placeholder') }}" />
                        @error('name') <p class="mt-1 text-xs text-danger">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <x-media-upload model="thumbnail" label="{{ __('admin.cake_flavors.thumbnail_label') }}"
                            hint="{{ __('admin.cake_flavors.thumbnail_hint') }}"
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-sm rounded-2xl bg-surface p-6 shadow-modal animate-scale-in">
                ...
                <h3 class="text-center font-display text-lg font-semibold text-foreground">
                    {{ __('admin.cake_flavors.delete_title') }}
                </h3>
                <p class="mt-2 text-center text-sm text-foreground-muted">{{ __('admin.cake_flavors.delete_message') }}</p>
