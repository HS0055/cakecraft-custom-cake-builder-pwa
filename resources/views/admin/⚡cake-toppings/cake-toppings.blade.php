<div class="animate-fade-in">
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="font-display text-2xl font-semibold text-foreground">{{ __('admin.cake_toppings.title') }}</h2>
            <p class="mt-1 text-sm text-foreground-muted">{{ __('admin.cake_toppings.subtitle') }}</p>
        </div>
        @can('create toppings')
            <button wire:click="openCreate" class="btn-base bg-primary text-primary-foreground hover:bg-primary-hover">
                ...
                {{ __('admin.cake_toppings.add_topping') }}
            </button>
        @endcan
    </div>
    ...
    <div class="mb-4 flex flex-col gap-4 sm:flex-row">
        <input wire:model.live.debounce.300ms="search" type="text"
            placeholder="{{ __('admin.cake_toppings.search_placeholder') }}" class="input-base w-full sm:max-w-sm" />
        <select wire:model.live="filter_category_id" class="select-base w-full sm:max-w-xs">
            <option value="">{{ __('admin.cake_toppings.all_categories') }}</option>
            @foreach ($categories as $category)
                <option value="{{ $category->id }}">{{ $category->name }}</option>
            @endforeach
        </select>
        @if ($search || $filter_category_id)
    ...
    <div class="card-base overflow-hidden">
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-border bg-surface-alt">
                        <th class="table-header px-6 py-3.5">{{ __('admin.cake_toppings.table_thumbnail') }}</th>
                        <th class="table-header px-6 py-3.5">{{ __('admin.cake_toppings.table_name') }}</th>
                        <th class="table-header px-6 py-3.5">{{ __('admin.cake_toppings.table_category') }}</th>
                        <th class="table-header px-6 py-3.5">{{ __('admin.cake_toppings.table_created') }}</th>
                        <th class="table-header px-6 py-3.5 text-end">{{ __('admin.common.actions') }}</th>
                    </tr>
                </thead>
                <tbody class="divide-y divide-border">
                    @forelse ($toppings as $topping)
                        <tr wire:key="topping-{{ $topping->id }}"
                            class="hover:bg-surface-alt/50 transition-colors duration-150">
                            <td class="px-6 py-4">
                                @if ($topping->getFirstMediaUrl('thumbnail'))
                                    <img src="{{ $topping->getFirstMediaUrl('thumbnail') }}" alt="{{ $topping->name }}"
                                        class="h-10 w-10 rounded-xl object-cover border border-border" />
                                @else
                                    <div
                                        class="flex h-10 w-10 items-center justify-center rounded-xl bg-surface-alt text-foreground-subtle">
                            ...
                            <td class="px-6 py-4 text-sm font-medium text-foreground">{{ $topping->name }}</td>
                            <td class="px-6 py-4 text-sm text-foreground-muted">
                                {{ $topping->category?->name ?? '-' }}
                            </td>
                            <td class="px-6 py-4 text-sm text-foreground-muted">{{ $topping->created_at->format('M d, Y') }}
                            </td>
                            <td class="px-6 py-4 text-end">
                                <div class="flex items-center justify-end gap-1">
                                    @can('update toppings')
                                        <button wire:click="openEdit({{ $topping->id }})"
                                    ...
                                    @endcan
                                    @can('delete toppings')
                                        <button wire:click="confirmDelete({{ $topping->id }})"
                    ...
                    @empty
                        <tr>
                            <td colspan="5" class="px-6 py-12 text-center text-sm text-foreground-muted">
                                {{ __('admin.cake_toppings.no_toppings') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        @if ($toppings->hasPages())
            <div class="border-t border-border px-6 py-4">{{ $toppings->links() }}</div>
        @endif
    </div>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-md rounded-2xl bg-surface p-6 shadow-modal animate-scale-in">
                <h3 class="font-display text-lg font-semibold text-foreground mb-5">
                    {{ $editingId ? __('admin.cake_toppings.edit_title') : __('admin.cake_toppings.create_title') }}
                </h3>
                <form wire:submit="save" class="space-y-4">
                    @php
                        $editingModel = $editingId ? $toppings->firstWhere('id', $editingId) : null;
                    @endphp
                    <div>
                        <label
                            class="mb-1.5 block text-sm font-medium text-foreground">{{ __('admin.cake_toppings.name_label') }}</label>
                        <input wire:model="name" type="text" class="input-base"
                            placeholder="{{ __('admin.cake_toppings.name_placeholder') }}" />
                        @error('name') <p class="mt-1 text-xs text-danger">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label
                            class="mb-1.5 block text-sm font-medium text-foreground">{{ __('admin.cake_toppings.category_label') }}</label>
                        <select wire:model="topping_category_id" class="select-base">
                            <option value="">{{ __('admin.cake_toppings.category_placeholder') }}</option>
                            @foreach ($categories as $category)
                                <option value="{{ $category->id }}">{{ $category->name }}</option>
                            @endforeach
                        </select>
                        @error('topping_category_id') <p class="mt-1 text-xs text-danger">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <x-media-upload model="thumbnail" label="{{ __('admin.cake_toppings.thumbnail_label') }}"
                            hint="{{ __('admin.cake_toppings.thumbnail_hint') }}"
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-sm rounded-2xl bg-surface p-6 shadow-modal animate-scale-in">
                ...
                <h3 class="text-center font-display text-lg font-semibold text-foreground">
                    {{ __('admin.cake_toppings.delete_title') }}
                </h3>
                <p class="mt-2 text-center text-sm text-foreground-muted">{{ __('admin.cake_toppings.delete_message') }}</p>
