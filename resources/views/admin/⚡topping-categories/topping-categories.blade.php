<div class="animate-fade-in">
    ...
    <div class="mb-6 flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <h2 class="font-display text-2xl font-semibold text-foreground">{{ __('admin.topping_categories.title') }}
            </h2>
            <p class="mt-1 text-sm text-foreground-muted">{{ __('admin.topping_categories.subtitle') }}</p>
        </div>
        @can('create topping categories')
            <button wire:click="openCreate" class="btn-base bg-primary text-primary-foreground hover:bg-primary-hover">
                ...
                {{ __('admin.topping_categories.add_category') }}
            </button>
        @endcan
    </div>
    ...
    <div class="mb-4">
        <input wire:model.live.debounce.300ms="search" type="text"
            placeholder="{{ __('admin.topping_categories.search_placeholder') }}"
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-border bg-surface-alt">
                        <th class="table-header px-6 py-3.5">{{ __('admin.topping_categories.table_name') }}</th>
                ...
                <tbody class="divide-y divide-border">
                    ...
                    @empty
                        <tr>
                            <td colspan="2" class="px-6 py-12 text-center text-sm text-foreground-muted">
                                ...
                                {{ __('admin.topping_categories.no_categories') }}
                            </td>
                        </tr>
                    @endforelse
                </tbody>
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4" x-data
            ...
            <div class="relative w-full max-w-lg rounded-2xl bg-surface p-6 shadow-modal animate-scale-in">
                <h3 class="font-display text-lg font-semibold text-foreground mb-5">
                    {{ $editingId ? __('admin.topping_categories.edit_title') : __('admin.topping_categories.create_title') }}
                </h3>

                <form wire:submit="save" class="space-y-4">
                    <div>
                        <label
                            class="mb-1.5 block text-sm font-medium text-foreground">{{ __('admin.topping_categories.name_label') }}</label>
                        <input wire:model="name" type="text" class="input-base"
                            placeholder="{{ __('admin.topping_categories.name_placeholder') }}" />
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center p-4">
            ...
            <div class="relative w-full max-w-sm rounded-2xl bg-surface p-6 shadow-modal animate-scale-in">
                ...
                <h3 class="text-center font-display text-lg font-semibold text-foreground">
                    {{ __('admin.topping_categories.delete_title') }}</h3>
                <p class="mt-2 text-center text-sm text-foreground-muted">
                    {{ __('admin.topping_categories.delete_message') }}</p>
